...
## Setup
X    <- cheem::amesHousing2018_NorthAmes[, 1:9]
Y    <- cheem::amesHousing2018_NorthAmes$SalePrice
clas <- cheem::amesHousing2018_NorthAmes$SubclassMS

## Model and predict
ames_train    <- data.matrix(X) %>%
  xgb.DMatrix(label = Y)
...
```{r, out.width="100%"}
global_view(ames_chm, primary_obs = 1, comparison_obs = 17,
...
# Other local explanations, (models & predictions)
...
## Setup
X    <- DALEX::apartments[, -1]
Y    <- DALEX::apartments$m2.price
clas <- DALEX::apartments$district

## Model and predict
apt_train    <- data.matrix(X) %>%
  xgb.DMatrix(label = Y)
apt_xgb_fit  <- xgboost(data = peng_train, max.depth = 3, nrounds = 25)
apt_xgb_pred <- predict(peng_xgb_fit, newdata = peng_train)

## SHAP
shp <- shapviz(peng_xgb_fit, X_pred = peng_train, X = X)
## Keep just the [n, p] local explanations
peng_xgb_shap <- shp$S
```
...
drag_lm_shap <- X[1:6, ] ## init a df of the same structure
for(i in 1:6){
  pps <- predict_parts_shap(drag_lm_exp, new_observation = dragons[i, ])
  ## Keep just the [n, p] local explanations
  drag_lm_shap[i, ] <- tapply(pps$contribution, pps$variable, mean, na.rm = TRUE)
}
```
